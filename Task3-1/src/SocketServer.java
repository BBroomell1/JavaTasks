...
public class SocketServer implements Runnable {
	...
	public void run() {
		...
		while (stopped() == false) {
			...
			this.threadPool.execute(new Task3Runnable(clientSocket, rootPath));
		} // End while
	}
...
/*
 * try { serverSocket = new ServerSocket(portNum); } catch(IOException ex) {
 * System.out.println("Server Socket could not connect to port number: " +
 * portNum); System.out.println(ex.getMessage()); }
 * 
 * while(true) { try { Socket clientSocket = serverSocket.accept(); new
 * Thread(new Task3Runnable(clientSocket)).start(); } catch(IOException ex) {
